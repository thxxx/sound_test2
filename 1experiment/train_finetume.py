...
def main():
    train_data_path = "../csv_files/train_mixed_captions.csv" # 이거만 Mix인지 아닌지 바꾸면 됨.
    eval_data_path = "../csv_files/eval_mixed_captions.csv" # 이거만 Mix인지 아닌지 바꾸면 됨.
    logger = Logger()
    logger.init()
    logger.logging("Start Finetuning without mixed audio")
    ...
    test_dataset = TestDataset(cfg, data_path="../csv_files/eval_mixed_captions.csv")
    ...
    for epoch in range(starting_epoch, cfg.num_train_epochs):
        ...
        torch.save(model.state_dict(), os.path.join(cfg.output_dir, f"base_{epoch}.pth"))
